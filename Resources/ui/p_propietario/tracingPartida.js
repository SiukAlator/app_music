...
function TracingPartida(openReady, nav, partida, piso, cal_ok, cal_nook) {
	var self;
	var nav;
	var content;
	...
	var trackerName = 'Profesional de terreno: Open Partida';
	...
	function construct() {
		...
		var extraHelp = ['En esta pantalla se modifica el porcentaje de avance en las diferentes unidades de control correspondiente a la partida filtrada.', 'Puedes cambiar el porcentaje presionando en la casilla de color o bien entrar al detalle de la partida presionando en la unidad de control.'];
			...
			if (listaTrackingUC[index].ID == idTrackingPresionado) {
				...
				if (flagExiste == true) {
					...
					Ti.Geolocation.getCurrentPosition(function(loc) {
						Ti.API.error("Ti.Geolocation.getCurrentPosition");
						if (loc.success == true) {
							Ti.API.error("moment.utc().valueOf():" + moment.utc().valueOf());
	...
	function openDetail() {
		...
		Ti.API.info("Se abre detalle:" + UCSeleccionada);
		var subTitle = Ti.App.Properties.getString('UC', null) + ' ' + UCSeleccionada;
		var Window = require('/ui/p_propietario/detallePartida');
		var idView = 1;
		var partidaSeleccionada = partida;
		if (Config.isAndroid) {
	...
	function ocultarPartidas100() {
		...
		for (var index in arrayboxPartida) {
			Ti.API.info("En el ciclo...");
			if (arrayboxPartida[index].porcentaje == '100') {
				Ti.API.info("En el if...");
				var respIndex = index;
	...
	function getData() {
		...
		xhr.getSeguimientoFiltroPartida(gotData, params);
	}

	function getDataLocal() {

		var varpartida = partida;

		var local = Config.locale;
		var code = 'tracking';
		var listaTracking_Complete = db.selectGENERAL(code);
		var listaTracking = listaTracking_Complete.values;

		for (var index in listaTracking) {

			if (listaTracking[index].multilang[local].name.toUpperCase() == partida.toUpperCase()) {
				if (piso == L('PlanningFilters_hintTextAll1')) {
					if (cal_ok == true && cal_nook == false) {
						if (listaTracking[index].info.quality == true) {
							addData(listaTracking[index], index);
						}
					} else if (cal_nook == true && cal_ok == false) {
						if (listaTracking[index].info.quality == false) {
							addData(listaTracking[index], index);
						}
					} else if (cal_nook == true && cal_ok == true) {

						addData(listaTracking[index], index);
					}

				} else {
					if (listaTracking[index].cuc[local].value.toUpperCase() == piso.toUpperCase()) {
						if (cal_ok == true && cal_nook == false) {
							if (listaTracking[index].info.quality == true) {
								addData(listaTracking[index], index);
							}
